  ...
  where
    ...
    getUserStat name = do
      let userQuery = Q.toUserTerm name
          ...
          commitQuery = Q.mkTerm "type" "Change"
          qf = QueryFlavor Monocle.Search.Syntax.Author

      userStats <- withFilter [userQuery] $ do
        reviewHisto <- withFilter [reviewQuery] $ Q.getHisto $ qf CreatedAt
        commitHisto <- withFilter [commitQuery] $ Q.getHisto $ qf UpdatedAt
