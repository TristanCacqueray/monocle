...
import Lentille (LentilleM, runLentilleM)
...
testLentille :: LentilleM a -> IO ()
testLentille lentille = do
  setEnv "CRAWLERS_API_KEY" "secret"
  setEnv "GITLAB_TOKEN" "42"
  res <- runLentilleM lentille
  case res of
    Left x -> error $ "Lentille failed: " <> show x
    Right _ -> pure ()

testGetStream :: Assertion
testGetStream = testLentille $ do
